        ...
        <Link href={`/study-detail/${data.studyId}`} className='py-[26px] px-[24px] bg-[#f9f9f9] rounded-[8px] flex flex-col justify-between gap-[10px] h-[340px]'>
            <div className='flex gap-4 flex-col h-full'>
                <div className='flex items-center justify-between'>
                    {data.status === '모집중' ? <div className='min-w-[60px] h-[30px] bg-[white] text-link-default rounded-[16px] flex items-center justify-center border border-link-default text-sm font-bold px-[12px]'>{data.status}</div> : <div className='min-w-[60px] h-[30px] bg-[white] text-gray-light rounded-[16px] flex items-center justify-center border border-gray-light text-sm font-bold px-[12px]'>{data.status}</div>}
                ...
                <div className='flex gap-2 flex-wrap items-center justify-start'>
                    {data.tagInquiryListResponse.map((tag) => (
